...
One run publishes one (workspace, domain, BI tool) combination, named
explicitly. Nothing is inferred from the git branch: which workspace a workflow
publishes, and whether it runs at all, are decided by the workflow's own
"paths:" filter rather than by this script.
