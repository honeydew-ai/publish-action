...
def test_require_env_fails(env: dict[str, str]) -> None:
    """The workspace is a required input; nothing is inferred from the git ref."""
    with mock.patch.dict(os.environ, env, clear=True), pytest.raises(SystemExit):
        publish.require_env("HONEYDEW_WORKSPACE")


def test_git_ref_is_never_read() -> None:
    """Removing branch-name detection means the ref cannot silently pick a workspace."""
